  ...
  ailmentSearch(ailment, zip, displayFn, displayErr) {
    ...
    googMaps.geoCodeAddress(zip)
      .then(response => {
        ...
        return $.get(`https://api.betterdoctor.com/2016-03-01/doctors?query=${ailment}+&location=${lat}%2C${lng}%2C%205&user_location=${lat}%2C${lng}&skip=0&limit=20&user_key=${apiKey}`);
      })
      .then(response => {
